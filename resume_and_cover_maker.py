...
- Gemini must return ALL education entries that appear in the original resume:
  * School / High School / Secondary
  * ALL Bachelor's degrees
  * ALL Master's degrees
  * ALL PhD / Doctoral degrees
  * Diplomas, PG diplomas, bootcamps, etc.
- The EDUCATION section in the PDF prints EVERY education item Gemini returns.
- Skills are allowed to change / be reorganized to match the job.
- Work experience bullets & tech stacks can be rewritten for the job.
- If no experience in the resume, Gemini invents plausible roles + dates.
- If any job / project has no dates, Gemini invents plausible non-clashing dates.
- Extra-curricular items added as last line in “Technical Skills and Certifications”.
"""
...
RESUME_PDF_PATH = BASE_DIR / "base_resume.pdf"
...
def call_gemini_all(resume_text: str, job_description: str, api_key: str):
    """
    ...
    # ---------- PROMPT (EDUCATION PART UPDATED PER YOUR REQUEST) ---------- #
    prompt = f"""
...
def format_education(education_list):
    ...
    This is where your "all education must be mentioned in the PDF" requirement
    is enforced on the LaTeX side. We don't filter out any non-empty entry.
    """
    lines = []
...
def format_skills(skills_list):
    ...
    for line in skills_list or []:
        if not line:
            continue
        s = re.sub(r'^\s*(skills?|technical skills?)\s*:\s*', '', str(line),
                   flags=re.IGNORECASE)
        parts = re.split(r"[,/]", s)
        for part in parts:
            token = part.strip()
...
def main():
    ...
    print("🎉 Done. One Gemini prompt, all education returned + shown in PDF, resume + cover letter ready.")
