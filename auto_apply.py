...
import resume_and_cover_maker2 as rcm
...
def _fallback_execute_actions_on_page(page, actions):
    ...
    Uses Playwright's page.mouse and page.keyboard.
    """
    try:
...
def call_gemini_for_page_recovery(
    ...
    Returns a dict parsed from Gemini's JSON response, expected shape:
      {
        "actions": [ { ... }, ... ],
    ...
    prompt = f"""
You are an assistant that helps a Selenium-based bot recover when it gets stuck on a web page.
Do NOT output any explanation. Output ONLY a single JSON object matching this schema:

{{"actions":[{{...}}], "comment":"short explanation"}}

Allowed action objects (in order):
- click: {{ "type":"click", "by":"css"|"xpath", "selector":"<selector>", "wait": <seconds_optional> }}
- type: {{ "type":"type", "by":"css"|"xpath", "selector":"<selector>", "text":"<text to type>", "clear": true|false, "wait": <seconds_optional> }}
- scroll: {{ "type":"scroll", "x": <int>, "y": <int>, "wait": <seconds_optional> }}
- press: {{ "type":"press", "by":"css"|"xpath", "selector":"<selector_optional>", "keys":"ENTER"|"TAB"|"ARROW_DOWN"|"ARROW_UP", "wait": <seconds_optional> }}
- wait: {{ "type":"wait", "seconds": <float> }}

Constraints:
- Use at most {max_actions} actions.
- Do not ask for passwords or type any secret you don't already know. If login is required, prefer clicking a visible 'Sign in' or 'Log in' button.
- Use CSS selectors if possible. If not, use an XPath.
- Return ONLY valid JSON. If unsure, return an empty actions list.

Phase: {phase}
Problem description:
{problem_description}

Visible text (truncated if long):
{visible_text[:4000]}

HTML (truncated, sanitized):
{(page_html[:8000] if page_html else "")}
"""
    ...
    try:
        ...
        if not m:
            debug("call_gemini_for_page_recovery: no JSON object found in Gemini response")
            return None
        json_blob = json.loads(m.group(0))

        if not isinstance(json_blob, dict) or "actions" not in json_blob or not isinstance(json_blob["actions"], list):
            debug("call_gemini_for_page_recovery: response JSON missing required 'actions' list.")
            return None

        allowed = {"click", "type", "scroll", "press", "wait"}
        sanitized_actions: List[Dict[str, Any]] = []

        for act in json_blob["actions"][:max_actions]:
            if not isinstance(act, dict) or "type" not in act:
                continue
            t = act["type"]
            if t not in allowed:
                continue
            safe_act: Dict[str, Any] = {"type": t}

            if t in {"click", "type", "press"}:
                by = act.get("by", "css")
                ...
                if not selector or len(selector) > 1000:
                    continue
                safe_act.update({"by": by, "selector": selector})
            ...
            if t == "type":
                ...
                if len(text) > 1000:
                    text = text[:1000]
                safe_act["text"] = text
                safe_act["clear"] = bool(act.get("clear", True))

            if t == "scroll":
                safe_act["x"] = int(act.get("x", 0))
                safe_act["y"] = int(act.get("y", 0))

            if t == "press":
                safe_act["keys"] = act.get("keys", "ENTER")

            if "wait" in act:
                try:
                    waitv = float(act["wait"])
                    safe_act["wait"] = min(max(waitv, 0.0), 10.0)
                except Exception:
                    pass

            sanitized_actions.append(safe_act)

        return {"actions": sanitized_actions, "comment": json_blob.get("comment", "")}
    except Exception as e:
        debug(f"call_gemini_for_page_recovery: exception parsing Gemini response: {e!r}")
        return None
...
def execute_gemini_actions(
    ...
    Supports: 'click','type','scroll','press','wait'.
    """
    if not actions:
    ...
    for idx, act in enumerate(actions):
        try:
            t = act.get("type")
            if t not in allowed:
                debug(f"execute_gemini_actions: skipping unsupported action type {t}")
                continue

            default_wait = float(act.get("wait", 0.35))

            if t == "wait":
                sec = float(act.get("seconds", default_wait))
                time.sleep(sec)
                continue
            ...
            if selector:
                try:
                    ...
                    debug(f"execute_gemini_actions: element not found for selector {selector} (by={by}): {e!r}")
                    elem = None
            ...
            if t == "click":
                if elem:
                    try:
                        ...
                        debug(f"execute_gemini_actions: click action failed: {e!r}")
                else:
                    debug("execute_gemini_actions: click action had no element; skipping.")
                continue

            if t == "type":
                text = act.get("text", "")
                clear_flag = act.get("clear", True)
                if elem:
                    try:
                        driver.execute_script("arguments[0].scrollIntoView({block:'center'});", elem)
                        if clear_flag:
                    ...
                    debug("execute_gemini_actions: type action had no element; skipping.")
                continue

            if t == "press":
                keys = act.get("keys", "ENTER")
                key_map = {
                    "ENTER": Keys.ENTER,
                    "TAB": Keys.TAB,
                ...
                k = key_map.get(str(keys).upper(), Keys.ENTER)
                ...
                try:
                    ...
                    debug(f"execute_gemini_actions: press action failed: {e!r}")
                continue
...
def try_gemini_page_recovery(
    ...
    """
    Capture page context, ask Gemini for a short sequence of actions, and execute them.
    Returns True if any recovery attempt executed.
    """
    ...
    # ensure we have a key: prefer argument, then gemini_api_key.txt
    api_key = (gemini_api_key or "").strip()
    if not api_key:
        api_key = resolve_gemini_api_key_from_env_or_disk() or ""
    ...
    for attempt in range(max_steps):
        ...
        debug(f"try_gemini_page_recovery: executing {len(actions)} actions from Gemini (comment={resp.get('comment')})")
...
def ensure_form_answers_applied_and_recover(
    ...
    Robust orchestrator that:
      - inspects visible fields and finds which are missing,
      - applies cached answers (memory-first),
      - calls Gemini once to fill remaining fields (if needed),
      - if errors or fields remain, asks Gemini for page recovery and executes it.

    This version is defensive: it tolerates differing signatures for helpers,
    handles Gemini key exhaustion by prompting once, and logs failures instead
    of throwing.
    """
    # Prefer GEMINI_API_KEY from environment if present
    ...
    try:
        ...
        reason = f"Failed to inspect form schema: {e!r}"
        try:
            try_gemini_page_recovery(
                ...
                phase=f"{mode}-inspect",
                max_steps=max_recovery_steps,
            )
            return True
        except Exception as e2:
            debug(f"ensure_form_answers_applied_and_recover: recovery also failed: {e2!r}")
            return False

    # ---- helper to test whether a field is filled (defensive) ----
        ...
        try:
            if field_type == "text":
                ...
                if el is None and name_attr:
                    try:
                        el = container.find_element(By.CSS_SELECTOR, f"input[name='{name_attr}']")
                    except Exception:
                        el = None
                if not el:
                    return False
            ...
            if field_type == "textarea":
                ...
                if el is None and name_attr:
                    try:
                        el = container.find_element(By.CSS_SELECTOR, f"textarea[name='{name_attr}']")
                    except Exception:
                        el = None
                if not el:
                    return False
            ...
            if field_type == "select":
                ...
                if el is None and name_attr:
                    try:
                        el = container.find_element(By.CSS_SELECTOR, f"select[name='{name_attr}']")
                    except Exception:
                        el = None
                if not el:
                    return False
    ...
    # ---- build missing map ----
    missing = { "text_fields": [], "textareas": [], "select_fields": [], "radio_groups": [], "checkboxes": [] }
    try:
        ...
        debug(f"ensure_form_answers_applied_and_recover: error while detecting missing fields: {e!r}")
        # fall back to conservative behaviour: assume something is missing and try recovery
        missing = { "text_fields": ["*"], "textareas": [], "select_fields": [], "radio_groups": [], "checkboxes": [] }

    total_missing = sum(len(v) for v in missing.values())
    if total_missing == 0:
        debug(f"ensure_form_answers_applied_and_recover: no empty visible fields for {mode} step {step_index+1}.")
        return True

    debug(f"ensure_form_answers_applied_and_recover: detected {total_missing} empty fields for {mode} step {step_index+1}: {{k: len(v) for k,v in missing.items()}}")

    # ---- attempt to load cached answers (robust against signature mismatch) ----
    cached = None
    try:
        # prefer keyword style first
        cached = load_form_answers_from_file(job_index=job_index, step_index=step_index, mode=mode, job_title=job_title)
    except TypeError as te:
        debug(f"load_form_answers_from_file TypeError with 4-arg call: {te!r}; trying alternate calls.")
        try:
            cached = load_form_answers_from_file(job_index, step_index, mode)
        except Exception as e2:
            debug(f"Second attempt to load_form_answers_from_file failed: {e2!r}")
            cached = None
    except Exception as e:
        debug(f"Unexpected error loading cached answers: {e!r}")
        cached = None

    # ---- apply cached answers for missing keys only ----
    if cached:
        to_apply = {"text_fields": {}, "textareas": {}, "select_fields": {}, "radio_groups": {}, "checkboxes": {}}
        for sect in to_apply.keys():
            sec = cached.get(sect) or {}
            for k in missing.get(sect, []):
                v = sec.get(k)
                if v is None:
                    continue
        ...
        if any(len(v) for v in to_apply.values()):
            debug("Applying cached answers for missing fields (memory-first).")
            try:
                apply_gemini_answers_to_form(driver, container, form_schema, to_apply)
            except Exception as e:
                debug(f"Error applying cached answers: {e!r}")

    # ---- re-evaluate still missing ----
    still_missing = {k: [] for k in missing.keys()}
    try:
        for it in form_schema.get("text_fields", []):
            k = it.get("field_key")
            if k and not _is_filled("text", it):
                still_missing["text_fields"].append(k)
        for it in form_schema.get("textareas", []):
            k = it.get("field_key")
            if k and not _is_filled("textarea", it):
                still_missing["textareas"].append(k)
        for it in form_schema.get("select_fields", []):
            k = it.get("field_key")
            if k and not _is_filled("select", it):
                still_missing["select_fields"].append(k)
        for it in form_schema.get("radio_groups", []):
            k = it.get("group_key")
            if k and not _is_filled("radio", it):
                still_missing["radio_groups"].append(k)
        for it in form_schema.get("checkboxes", []):
            k = it.get("box_key")
            if k and not _is_filled("checkbox", it):
                still_missing["checkboxes"].append(k)
    except Exception as e:
        debug(f"ensure_form_answers_applied_and_recover: re-evaluate missing failed: {e!r}")

    total_still = sum(len(v) for v in still_missing.values())
    if total_still == 0:
        debug("Cached answers covered all missing fields.")
        return True

    # ---- prepare for Gemini call ----
    # Ensure we have a key or prompt user (speak once)
    if not gemini_api_key:
        debug("No Gemini API key available; will prompt user for a key for this run.")
        new_key = prompt_for_new_gemini_key()
        if new_key:
            gemini_api_key = new_key.strip()
        else:
            reason = f"Missing {total_still} fields but no Gemini key available."
            try:
                try_gemini_page_recovery(driver, container, gemini_api_key or "", reason, phase=f"{mode}-no-key", max_steps=max_recovery_steps)
                return True
            except Exception as e:
                debug(f"Recovery attempt failed: {e!r}")
                return False

    debug(f"Calling Gemini to fill {total_still} remaining fields (one shot).")
    existing_answers = cached or {"text_fields": {}, "textareas": {}, "select_fields": {}, "radio_groups": {}, "checkboxes": {}}

    # ---- call Gemini (tolerant to signature differences) ----
    gemini_out = None
    try:
        try:
            ...
            # older signature fallback: without missing_keys
            gemini_out = call_gemini_for_form_answers(
                resume_text=resume_plain,
                applicant_profile=applicant_profile,
                job_description=job_description,
        ...
        msg = str(e).lower()
        debug(f"Gemini call failed: {e!r}")
        # Detect key/quota related messages and prompt once more for a key if appropriate
        key_issue_tokens = ["apikey", "api key", "quota", "exhaust", "exceeded", "permission", "403", "401", "billing"]
        if any(tok in msg for tok in key_issue_tokens):
            # ask user once more for a key
            new_key = prompt_for_new_gemini_key()
            if new_key:
                gemini_api_key = new_key.strip()
                try:
                    ...
                    debug(f"Retry Gemini call after new key failed: {e2!r}")
            else:
                debug("User declined to provide new Gemini key.")
        else:
            # non-key failure — try page recovery
            reason = f"Gemini call to fill missing fields failed: {e!r}"
            try:
                try_gemini_page_recovery(driver, container, gemini_api_key or "", reason, phase=f"{mode}-gemini-failed", max_steps=max_recovery_steps)
                return True
            except Exception as e2:
                debug(f"Recovery attempt after Gemini failure also failed: {e2!r}")
                return False

    if not gemini_out:
        debug("No Gemini output produced; attempting page recovery.")
        reason = "Gemini produced no output for missing fields."
        try:
            try_gemini_page_recovery(driver, container, gemini_api_key or "", reason, phase=f"{mode}-no-gemini-output", max_steps=max_recovery_steps)
            return True
        except Exception as e:
            debug(f"Recovery attempt failed: {e!r}")
            return False

    # ---- merge + save answers (defensive) ----
    try:
        merged = merge_gemini_answer_dicts(existing_answers, gemini_out) if existing_answers else gemini_out
    except Exception as e:
        debug(f"merge_gemini_answer_dicts failed: {e!r}; using gemini_out directly.")
        merged = gemini_out

    try:
        save_form_answers_to_file(job_index=job_index, step_index=step_index, job_title=job_title, mode=mode, answers=merged)
    except Exception as e:
        debug(f"Failed to save Gemini answers: {e!r}")

    # ---- apply only relevant answers for still_missing keys ----
    to_apply_after = {"text_fields": {}, "textareas": {}, "select_fields": {}, "radio_groups": {}, "checkboxes": {}}
    for sect in to_apply_after.keys():
        section = merged.get(sect) or {}
        for k in still_missing.get(sect, []):
            v = section.get(k)
            if v is None:
                continue
            to_apply_after[sect][k] = v

    try:
        apply_gemini_answers_to_form(driver, container, form_schema, to_apply_after)
    except Exception as e:
        debug(f"Error applying Gemini answers: {e!r}")

    # ---- final check and recovery if still missing ----
    final_missing = {k: [] for k in missing.keys()}
    try:
        for it in form_schema.get("text_fields", []):
            k = it.get("field_key")
            if k and not _is_filled("text", it):
                final_missing["text_fields"].append(k)
        for it in form_schema.get("textareas", []):
            k = it.get("field_key")
            if k and not _is_filled("textarea", it):
                final_missing["textareas"].append(k)
        for it in form_schema.get("select_fields", []):
            k = it.get("field_key")
            if k and not _is_filled("select", it):
                final_missing["select_fields"].append(k)
        for it in form_schema.get("radio_groups", []):
            k = it.get("group_key")
            if k and not _is_filled("radio", it):
                final_missing["radio_groups"].append(k)
        for it in form_schema.get("checkboxes", []):
            k = it.get("box_key")
            if k and not _is_filled("checkbox", it):
                final_missing["checkboxes"].append(k)
    except Exception as e:
        debug(f"Final missing re-check failed: {e!r}")

    total_final = sum(len(v) for v in final_missing.values())
    if total_final == 0:
        debug("All missing fields filled after Gemini application.")
        return True

    debug(f"After applying Gemini answers, {total_final} fields remain empty: {{k: len(v) for k,v in final_missing.items()}}")
    reason = f"After attempting to auto-fill, {total_final} fields remain empty; requesting page recovery."
    try:
        try_gemini_page_recovery(driver, container, gemini_api_key or "", reason, phase=f"{mode}-post-fill", max_steps=max_recovery_steps)
        return True
    except Exception as e:
        debug(f"ensure_form_answers_applied_and_recover: recovery failed: {e!r}")
        return False
...
def mouse_action(
    ...
    """
    Perform a mouse-like action on the page.
    ...
    Args:
      ...
      action: one of:
          ...
          - "move_to"
          - "scroll"
      locator_type: how to find the element (if needed), one of:
          - "css"
          - "xpath"
          - "id"
          - "text"
          - None (for actions that don't need an element, e.g., scroll)
      locator: the locator string (CSS selector, XPath, element id, or text fragment).
      offset_x: for "scroll", horizontal pixel offset (can be negative).
      offset_y: for "scroll", vertical pixel offset (can be negative).
    """
    action_norm = (action or "").strip().lower()
    if not action_norm:
        debug("mouse_action: empty action, doing nothing.")
        return

    def find_element_by_locator(
        loc_type: Optional[str], loc_value: Optional[str]
    ):
        ...
        try:
            ...
            debug(
                f"mouse_action: could not find element for "
                f"locator_type={loc_type_norm} locator={loc_val}: {e}"
            )
            return None
        return None

    # Scroll actions may not need an element
    ...
    if elem is None:
        debug(
            f"mouse_action: no element found for action={action_norm}, "
    ...
    try:
        ...
        if action_norm == "click":
            debug(f"mouse_action: clicking element ({locator_type}={locator})")
            actions.move_to_element(elem).click().perform()
        elif action_norm == "double_click":
            debug(f"mouse_action: double-clicking element ({locator_type}={locator})")
            actions.move_to_element(elem).double_click().perform()
        elif action_norm == "right_click":
            debug(f"mouse_action: right-clicking element ({locator_type}={locator})")
            actions.move_to_element(elem).context_click().perform()
        elif action_norm == "move_to":
            debug(f"mouse_action: moving to element ({locator_type}={locator})")
        ...
        debug(f"mouse_action: action '{action_norm}' failed: {e}")
...
def call_gemini_for_recovery_actions(
    ...
    """
    Ask Gemini for a sequence of mouse_action / keyboard_action commands
    to help the bot recover from being stuck on a page.
    ...
    # ---- SAFE CALL: do NOT pass max_chars here ----
    try:
        page_ctx = capture_page_context_for_gemini(driver, container=container)
    except TypeError:
        # Fallback: older versions might take only (driver)
        try:
    ...
    # page_ctx may be a dict (screenshot/html/text) or a plain string
    if isinstance(page_ctx, dict):
        ...
        # treat as already‑formatted context string
...
- action: one of "click", "double_click", "right_click", "move_to", "scroll"
- locator_type: one of "css", "xpath", "id", "text", or null if not needed
- locator: selector string (CSS, XPath, id, or text fragment to search in visible text)
- offset_x / offset_y: integers for scroll (pixels). Positive = scroll right / down, negative = left / up.
""".strip()

    keyboard_fn_desc = """
keyboard_action(driver, action, locator_type=None, locator=None, text=None, key=None)

- action: one of "type" or "press"
- locator_type: "css", "xpath", "id", "text", or null (for global key presses)
- locator: selector string for the element when typing
- text: text to type when action == "type"
- key: special key name when action == "press", like "ENTER", "TAB", "ESC", "SPACE"
""".strip()

    prompt = f"""
You are a recovery controller for a Selenium-based job application bot.

The bot got STUCK during phase: {phase}
Error or problem description:
{problem_description}

You CANNOT execute code directly. Instead, the bot can call exactly TWO local Python
helpers which you must plan for:
...
Your job: return a SMALL ordered list of steps, where each step can optionally
include one mouse_action() command, one keyboard_action() command, or both.
...
{{
  "steps": [
    {{
      "mouse": {{
        "action": "click" | "double_click" | "right_click" | "move_to" | "scroll",
        "locator_type": "css" | "xpath" | "id" | "text" | null,
        "locator": "<selector or short text or id>",
        "offset_x": <int or null>,
        "offset_y": <int or null>
      }} | null,
      "keyboard": {{
        "action": "type" | "press",
        "locator_type": "css" | "xpath" | "id" | "text" | null,
        "locator": "<selector or text or id or null>",
        "text": "<text to type or null>",
        "key": "<special key name or null>"
      }} | null
    }},
    ...
  ]
...
""".strip()
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not raw:
                debug("Gemini recovery call returned empty text; using empty plan.")
                return empty_plan()
            ...
            try:
                ...
                debug("Gemini recovery response is not valid JSON; using empty plan.")
                return empty_plan()

            if not isinstance(data, dict):
                debug("Gemini recovery response is not a JSON object; using empty plan.")
                return empty_plan()

            steps = data.get("steps")
            if not isinstance(steps, list):
                debug("Gemini recovery response missing 'steps' list; using empty plan.")
                return empty_plan()
            ...
            for step in steps:
                ...
                if mouse_cmd is not None and not isinstance(mouse_cmd, dict):
                    mouse_cmd = None
                if keyboard_cmd is not None and not isinstance(keyboard_cmd, dict):
                    keyboard_cmd = None
                normalized_steps.append({"mouse": mouse_cmd, "keyboard": keyboard_cmd})
            ...
            last_error = e
            msg = (str(e) or "").lower()
            debug(f"Gemini recovery call attempt {attempt+1} failed: {e!r}")

            # Handle exhausted / invalid key by asking you for a new one
            key_problem_tokens = [
                "api key",
                "apikey",
                "invalid api key",
            ...
            is_key_problem = any(tok in msg for tok in key_problem_tokens)
            ...
            if is_key_problem:
                ...
                if new_key:
                    ...
                    break  # stop retrying if you don't give a new key

            time.sleep(1.5)

    if last_error:
        debug(f"Gemini recovery call ultimately failed: {last_error!r}")
    return {"steps": []}
...
def try_gemini_page_recovery(
    ...
    """
    High-level helper: when the bot is stuck on some page or form step,
    call Gemini to propose mouse/keyboard actions, then execute them.

    This is best-effort only; if anything fails, the error is logged and
    the original flow continues.
    """
    if not gemini_api_key:
        debug("try_gemini_page_recovery: no Gemini API key; cannot attempt recovery.")
        return

    try:
        plan = call_gemini_for_recovery_actions(
            driver=driver,
            container=container,
            api_key=gemini_api_key,
            problem_description=problem_description,
        ...
        debug(f"try_gemini_page_recovery: Gemini planning failed: {e!r}")
        return

    try:
        execute_recovery_plan(driver, plan, max_steps=max_steps)
    except Exception as e:
        debug(f"try_gemini_page_recovery: executing plan failed: {e!r}")
...
""".strip()
    ...
    allowed_functions = {"click_element", "type_text", "scroll_page", "press_key", "none"}
...
def login_to_linkedin(driver: webdriver.Chrome, email: str, password: str) -> None:
    debug("Logging in to LinkedIn...")
    driver.get("https://www.linkedin.com/login")

    # Fill username / password as before
    try:
    ...
    # First try: normal wait for the logged‑in UI (global nav search bar)
    try:
        WebDriverWait(driver, 20).until(
            EC.presence_of_element_located((By.ID, "global-nav-search"))
        )
        # Login looks good, nothing else to do
        return
    except TimeoutException:
        debug(
            "Login did not appear to succeed on first attempt; "
            "trying Gemini-based recovery on the LinkedIn login page."
        )

        # Resolve Gemini key from env or disk (if available)
        gemini_api_key = resolve_gemini_api_key_from_env_or_disk() or ""
        if not gemini_api_key:
            debug("No Gemini API key available for login recovery; skipping Gemini recovery.")
        else:
            # Use the existing Gemini page-recovery helper to suggest mouse/keyboard actions
            try:
                try:
                ...
                try_gemini_page_recovery(
                    ...
                    gemini_api_key=gemini_api_key,
                    problem_description=(
                        "After submitting the LinkedIn login form, the page did not "
                        "navigate to the home feed or show the global navigation search."
                    ),
                    phase="login",
                    max_steps=3,
                )
            except Exception as e:
                debug(f"Gemini-based login recovery failed: {e!r}")

    # After Gemini-assisted attempt (or if no key), check one more time
    try:
        WebDriverWait(driver, 20).until(
            EC.presence_of_element_located((By.ID, "global-nav-search"))
        )
        debug("Login appears to have succeeded after Gemini recovery.")
    except TimeoutException:
        raise RuntimeError(
            "Login did not appear to succeed (even after Gemini recovery). "
...
def find_easy_apply_button(driver: webdriver.Chrome):
    """
    Try to locate the BLUE 'Easy Apply' button for the currently opened job.

    We prioritise:
      - the top-card Easy Apply button in the job details pane, and
      - buttons styled as LinkedIn primary blue.
    """
    candidates = []

    # Most specific selectors first (what your screenshot shows)
    selectors = [
        # Top card in job details pane (usually blue)
        "button.jobs-apply-button.jobs-apply-button--top-card.artdeco-button--primary",
        "button.jobs-apply-button.jobs-apply-button--top-card",
        # Any primary 'Apply' button
        "button.jobs-apply-button.artdeco-button--primary",
        # Generic LinkedIn apply buttons
    ...
    # Also match by visible text / aria-label in case classes change
    try:
        text_matches = driver.find_elements(
            By.XPATH,
    ...
    # Deduplicate and keep only buttons that clearly say "Easy Apply"
    def _has_easy_apply_label(btn) -> bool:
        try:
            txt = (btn.text or "").strip().lower()
            aria = (btn.get_attribute("aria-label") or "").strip().lower()
            return "easy apply" in txt or "easy apply" in aria
        except Exception:
            return False
    ...
    for btn in candidates:
        ...
        if key in seen:
            continue
        seen.add(key)
        if not _has_easy_apply_label(btn):
            continue
        unique.append(btn)

    # Keep only visible + enabled
    visible = []
    ...
    def is_blue_primary(button) -> bool:
        """Check if button looks like the blue primary Easy Apply."""
        try:
            classes = (button.get_attribute("class") or "").split()
            if "artdeco-button--primary" in classes:
                return True

            # Fallback: inspect background color
            bg = (button.value_of_css_property("background-color") or "")
            bg = bg.replace(" ", "").lower()
            # LinkedIn blues (rgb/rgba) commonly around these values
            if "10,102,194" in bg or "0,119,181" in bg:
                return True
        except Exception:
            pass
        return False

    def on_right_half(button) -> bool:
        """Prefer buttons on the right-hand job details pane."""
        try:
            size = driver.get_window_size()
            mid_x = size.get("width", 0) // 2
            loc = button.location or {}
            return loc.get("x", 0) >= mid_x
        except Exception:
            return False

    blue_buttons = [b for b in visible if is_blue_primary(b)]

    # 1) Prefer blue buttons on the right half
    for collection in (blue_buttons, visible):
        if not collection:
            continue

        right_side = [b for b in collection if on_right_half(b)]
        if right_side:
            return right_side[0]

        # Otherwise, just take the first from this collection
        return collection[0]
...
def find_external_apply_button(driver: webdriver.Chrome):
    """
    Try to locate an 'Apply on company site' / 'Apply' external button.
    This is heuristic and may need adjustment for specific job pages.
    """
    candidates = []
    texts = [
    ...
    for t in texts:
        try:
            elems = driver.find_elements(
                By.XPATH,
                f"//a[contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), '{t.lower()}')]"
            )
            candidates.extend(elems)
        except Exception:
            pass
        try:
            elems = driver.find_elements(
                By.XPATH,
                f"//button[contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), '{t.lower()}')]"
            )
            candidates.extend(elems)
        except Exception:
            pass
    ...
    for el in candidates:
        try:
            label = safe_text(el).strip().lower()
...
def click_next_or_submit_in_container(container, mode: str = "easy") -> bool:
    """
    Click a 'Next', 'Continue', 'Review', 'Apply', or 'Submit' button
    inside the given container (dialog or full page).

    Returns True if something was clicked, False if nothing matched.
    """
    labels = [
        "Submit application",
        "Submit",
...
def save_gemini_api_key_to_disk(api_key: str) -> None:
    ...
    NOTE: gemini_api_key.txt is the ONLY persistent source of the key.
    """
    api_key = (api_key or "").strip()
    if not api_key:
        return
    try:
        ...
        debug(f"Failed to save Gemini API key to disk: {e}")


def load_gemini_api_key_from_disk() -> Optional[str]:
    """
    Load a Gemini API key from gemini_api_key.txt if it exists.
    Returns the key string or None if the file is missing/empty.
    """
    try:
        ...
        debug(f"Failed to read Gemini API key from disk: {e}")
    return None


def resolve_gemini_api_key_from_env_or_disk() -> Optional[str]:
    """
    Compatibility wrapper: now ALWAYS loads the Gemini API key from
    gemini_api_key.txt and ignores any environment variables.
    """
    key = load_gemini_api_key_from_disk()
    if not key:
        debug(
            "No Gemini API key found in gemini_api_key.txt. "
            "Gemini-assisted features will be disabled until you save a key there."
        )
    return key
...
def prompt_for_new_gemini_key() -> Optional[str]:
    """
    Prompt the user (voice + console) for a new Gemini API key when
    the current one is exhausted/invalid.

    The new key is written to gemini_api_key.txt and also returned
    so the caller can immediately retry with it.
    """
    msg = (
        ...
        "Press ENTER without typing anything to skip (no Gemini for this step).\n"
    )
    print(msg, flush=True)

    if not getattr(prompt_for_new_gemini_key, "_spoken_once", False):
        try:
            speak("Gemini key exhausted. Please create a new valid key and paste it into the console.")
        except Exception:
            pass
        setattr(prompt_for_new_gemini_key, "_spoken_once", True)
...
def apply_gemini_answers_to_form(
    ...
    - Handles text, textareas, dropdowns, radio MCQ, checkboxes.
    """
    ...
    for field in form_schema.get("select_fields", []):
        ...
        try:
            ...
            chosen = None
            for opt in options:
                if opt.strip().lower() == desired_str:
                    chosen = opt
                    break
            if chosen is None:
                for opt in options:
                    low = opt.strip().lower()
                    if desired_str in low or low in desired_str:
                        chosen = opt
                        break
    ...
    # ---------------- RADIO GROUPS (MCQ / SINGLE CHOICE) ---------------- #
    ...
    for group in form_schema.get("radio_groups", []):
        ...
        for meta in inputs_meta:
            ...
            if rid:
                try:
                    elem = container.find_element(By.ID, rid)
                except Exception:
                    elem = None

            if elem is None and rname:
                try:
                    radios_by_name = container.find_elements(
                        By.CSS_SELECTOR, f"input[type='radio'][name='{rname}']"
                    )
                except Exception:
                    radios_by_name = []
                radios.extend(radios_by_name)
        ...
        for r in radios:
            ...
            if key_id in seen_ids:
                continue
            seen_ids.add(key_id)
            unique_radios.append(r)

        for r in unique_radios:
            try:
                if not r.is_displayed() or not r.is_enabled():
                    continue

                label = (get_label_for_element(container, r) or "").strip().lower()
                val = (r.get_attribute("value") or "").strip().lower()

                if (
                    desired_str == label
                    or desired_str == val
                    or desired_str in label
                    or desired_str in val
                    or (label and label in desired_str)
                    or (val and val in desired_str)
                ):
                    driver.execute_script(
...
def answer_form_with_gemini_for_container(
    ...
    """
    # Prefer GEMINI_API_KEY from environment if available
    disk_key = load_gemini_api_key_from_disk()
    if disk_key:
        gemini_api_key = disk_key
...
def easy_apply_for_job(
    ...
    """
    Run a Gemini-assisted Easy Apply flow for the currently opened job.

    Behaviour:

      1) Click the job's Easy Apply button.
      2) Wait for a REAL Easy Apply UI:
            - a modal (dialog / artdeco / jobs-easy-apply-modal), or
            - a full-page apply flow (/jobs/apply or *easyapply* in URL).
         If that does not appear, we abort Easy Apply for this job.
      3) Once the UI is confirmed, generate tailored resume + cover letter.
      4) For each step:
            - upload the (optionally merged) resume if file inputs exist,
            - fill basic fields from your profile,
            - let Gemini answer the step questions,
            - click Next / Submit / Apply.
         If we cannot find a Next/Submit/Apply button, we optionally ask Gemini
         for mouse/keyboard commands to try to recover.
      5) When the Easy Apply UI disappears, we assume submission/redirect.
    """

    # --------- helpers ---------

    def find_easy_apply_container():
        """
        Try to locate a specific Easy Apply container:
          - modal dialog (role='dialog', aria-modal, jobs-easy-apply-modal)
          - or a full-page apply form (/jobs/apply/).
        """
        selectors = [
            "div[role='dialog']",
            "div[aria-modal='true']",
            "div.jobs-easy-apply-modal",
            "div.artdeco-modal",
            "form[data-test-form-type='easy-apply']",
        ]
        for sel in selectors:
            try:
                elems = driver.find_elements(By.CSS_SELECTOR, sel)
    ...
    def get_any_container():
        # Only treat a real Easy Apply modal/page as the container.
        return find_easy_apply_container()

    def wait_for_easy_apply_container(timeout: int = 12):
        """
        Wait up to `timeout` seconds for the Easy Apply UI to appear.
        Returns the container if found, else None.
        """
        try:
    ...
    def robust_click_easy_apply(btn) -> bool:
        """
        Click Easy Apply in a way that looks like a real user:
          - scroll into view
          - wait for it to be clickable
          - move mouse + click via ActionChains
        ...
        try:
            ...
            debug("Easy Apply button never became clickable.")
            return False
        ...
        try:
            ...
            try:
                ...
                debug(f"JavaScript click on Easy Apply failed: {e}")
                return False
    ...
    # Wait for some Easy Apply UI to appear
    ...
    # --------- generate tailored docs for this job ---------

    try:
        tailored_resume = generate_tailored_docs(base_resume_pdf, job_desc_path)
    except Exception as e:
        debug(f"Failed to generate tailored resume for Easy Apply job #{job_index+1}: {e}")
        return False

    # Figure out cover letter + merged resume for this job
    ...
    for step in range(max_steps):
        debug("Easy Apply: processing step")


        # If a 'r u a robot' / CAPTCHA page shows up, speak and wait for human
        ...
        # Try to upload the tailored resume (merged with cover letter) on this step
        try:
            ...
            debug(f"Error uploading resume in Easy Apply: {e}")

        # Fill simple fields from your stored profile
        try:
            fill_basic_fields_in_container(container, profile)
        except Exception as e:
            debug(f"Error filling basic fields in Easy Apply: {e}")

        # Let Gemini answer this step's questions
        # Let Gemini/memory fill this step's questions only if needed
                # Let Gemini/memory fill this step's questions (including MCQ / dropdown) and recover if needed
        try:
            ...
            debug(f"Gemini assistance (with recovery) failed on Easy Apply step {step + 1}: {e}")
        ...
        try:
            ...
            debug(f"Error clicking Next/Submit in Easy Apply: {e}")

        if not clicked:
            debug("No Next/Submit/Apply button found; attempting Gemini-based page recovery for Easy Apply.")
            try:
                try_gemini_page_recovery(
                    driver=driver,
                    container=container,
                ...
                debug(f"Gemini-based page recovery failed on Easy Apply: {e}")

            # After recovery attempt, check once more for a button
            try:
                container = get_any_container() or container
                clicked = click_next_or_submit_in_container(container, mode="easy")
            except Exception as e:
                debug(f"Error re-trying Next/Submit after Gemini recovery: {e}")
                clicked = False
        ...
        # ===== NEW: detect "button clicked but form still has empty/unselected fields" =====
        stuck_container = None
        try:
            stuck_container = get_any_container()
        except Exception:
            stuck_container = None

        if stuck_container is not None:
            try:
                if is_any_field_empty(stuck_container):
                    debug(
                        "After clicking Next/Submit, Easy Apply step still has empty or unselected "
                        "fields (including MCQs/dropdowns); invoking Gemini recovery for stuck page."
                    )
                    try:
                        try_gemini_page_recovery(
                            driver=driver,
                            ...
                            gemini_api_key=gemini_api_key,
                            problem_description=(
                                "Clicked Next/Submit on an Easy Apply step but the page did not "
                                "advance, and some required fields / MCQs / dropdowns still look empty."
                            ),
                            phase="easy-apply-stuck",
                            max_steps=3,
                        )
                    except Exception as e:
                        debug(f"Gemini-based recovery for stuck Easy Apply step failed: {e!r}")
            except Exception as e:
                debug(f"Error while checking for remaining empty fields after Easy Apply click: {e!r}")
...
def external_apply_for_job(
    ...
    """
    Try to apply via an external company site.

    Behaviour:
      1) Click the external 'Apply' / 'Apply on company site' button.
      2) Wait for a new tab/window to open and switch to it.
      3) Generate tailored resume + cover letter for this job.
      4) For each step on the external site:
           - upload resume / cover letter when file inputs exist,
           - fill simple profile fields,
           - let Gemini answer more complex questions,
           - click Next / Submit / Apply.
         If a Next/Submit/Apply button cannot be found, we optionally
         ask Gemini for mouse/keyboard recovery steps.
      5) Close the external tab and return to LinkedIn.
    """
    # Remember LinkedIn window
    try:
    ...
    try:
        ...
        debug(f"Failed to click external Apply button: {e}")
        return False
    ...
    try:
        ...
        debug(f"Could not switch to external apply window: {e}")
        return False

    # Give the external page a moment to load
    time.sleep(5.0)

    # Generate tailored resume / cover letter for external job
    try:
        tailored_resume = generate_tailored_docs(base_resume_pdf, job_desc_path)
    except Exception as e:
        debug(f"Failed to generate tailored resume for external job #{job_index+1}: {e}")
        # Attempt to cleanly return to LinkedIn
        try:
            driver.close()
    ...
    for step in range(max_steps):
        debug("External apply: processing step")


        # Pause if the external site is showing a CAPTCHA / 'I'm not a robot'
        if not detect_and_wait_for_captcha(driver):
            debug("CAPTCHA not solved in time; stopping external apply for this job.")
            break

        try:
            container = driver.find_element(By.TAG_NAME, "body")
        except Exception as e:
            debug(f"Could not find <body> on external site: {e}")
            break


        # Try uploading tailored resume (merged with cover letter)
        try:
            upload_resume_in_container(
                container,
                tailored_resume,
            ...
            debug(f"Error uploading resume on external site: {e}")

        # Fill simple fields from your profile
        try:
            fill_basic_fields_in_container(container, profile)
        except Exception as e:
            debug(f"Error filling basic fields on external site: {e}")

        # Gemini answers for this page's questions
        try:
            ensure_form_answers_applied(
                driver=driver,
                container=container,
                resume_plain=resume_plain,
                applicant_profile=profile,
            ...
            debug(f"Gemini assistance failed on external step {step+1}: {e}")

        # Click next/submit on the external page
        clicked = False
        try:
            clicked = click_next_or_submit_in_container(container, mode="external")
        except Exception as e:
            debug(f"Error clicking Next/Submit on external site: {e}")
            clicked = False
        ...
        if clicked:
            ...
            # Optional: check if external page still has empty/unselected fields → treat as stuck
            try:
            ...
            if ext_container is not None:
                try:
                    if is_any_field_empty(ext_container):
                        debug(
                            "External apply: after clicking Next/Submit, some fields/MCQs/dropdowns "
                            "still appear empty; invoking Gemini recovery for stuck external step."
                        )
                        try:
                            try_gemini_page_recovery(
                                driver=driver,
                                ...
                                gemini_api_key=gemini_api_key,
                                problem_description=(
                                    "Clicked Next/Submit on an external apply step but the page did not "
                                    "seem to advance and some fields/MCQs/dropdowns are still empty."
                                ),
                                phase="external-apply-stuck",
                                max_steps=3,
                            )
                        except Exception as e:
                            debug(f"Gemini-based recovery for stuck external step failed: {e!r}")
                except Exception as e:
                    debug(f"Error while checking for remaining empty fields on external site: {e!r}")

        else:
            debug("No obvious submit/next/apply button clicked on external step; attempting Gemini-based page recovery.")
            try:
                try_gemini_page_recovery(
                    driver=driver,
                    container=container,
                ...
                debug(f"Gemini-based page recovery failed on external apply: {e}")
            ...
            try:
                ...
                debug(f"Error re-trying Next/Submit after Gemini recovery on external site: {e}")
                clicked = False
            ...
            if clicked:
                ...
                debug("Still no submit/next/apply button after Gemini recovery; stopping external apply flow.")
                break

    # Close external tab and switch back to LinkedIn
    try:
        driver.close()
        driver.switch_to.window(original_window)
    except Exception:
    ...
    if clicked_any_submit:
        debug(
            "External apply flow executed (clicked at least one Apply/Submit). "
            "Please verify the application status manually."
        )
    else:
        debug("External apply flow did not manage to click any Apply/Submit buttons.")

    return clicked_any_submit
...
def process_jobs(
    ...
    - ✅ Now only counts a job as successfully auto-applied if either:
         * Easy Apply / external flow reports success, OR
         * LinkedIn shows an 'Application submitted / You applied' confirmation.
    """
    initial_cards = find_job_cards(driver, log=True)
    debug(f"Found {len(initial_cards)} job cards in search results.")
    ...
    for idx in range(len(initial_cards)):
        ...
        # ---- Extra check: does LinkedIn show "Application submitted"? ----
        ...
        success = bool(easy_success or external_success or linkedin_confirmed)
    ...
    # Clean final summary on the terminal
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    # -------- Gemini API key: read only from gemini_api_key.txt --------
    gemini_api_key = ""
    try:
        key_file = BASE_DIR / "gemini_api_key.txt"
        if key_file.exists():
            ...
            if key_text:
                ...
                debug(f"Loaded Gemini API key from {key_file}")
    except Exception as e:
        debug(f"Failed to read gemini_api_key.txt: {e!r}")


    if not gemini_api_key:
        debug(
            "No GEMINI_API_KEY set and no gemini_api_key.txt found. "
            "Gemini-assisted filling will be disabled until you provide a key at runtime."
        )
        # This is okay: call_gemini_for_form_answers() will notice there is no key
        # and simply skip AI-assisted filling until you paste a new key when prompted.
